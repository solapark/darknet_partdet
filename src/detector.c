...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, int mjpeg_port, int show_imgs)
{
    ...
    while (get_current_batch(net) < net.max_batches) {
        ...
        */
        ...
        //if (i % 100 == 0) {
        if (i == 100 || i >= (iter_save + 1000) || i % 1000 == 0) {
            iter_save = i;
#ifdef GPU
...
void test_detector_classification(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile)
{
    ...
    while (1) {
        ...
        if (outfile) {
            ...
            fwrite(buff, sizeof(char), strlen(buff), json_file);
            int i;
            for (i = 0; i < nboxes; ++i) {
                char buff[1024];
                int class_id = -1;
                ...
                if (class_id >= 0) {
                    sprintf(buff, " %d", class_id);
                    fwrite(buff, sizeof(char), strlen(buff), json_file);
                }
            }
            fwrite("\n", sizeof(char), 1, json_file);
        }
...
void run_detector(int argc, char **argv)
{
    ...
    if (0 == strcmp(argv[2], "test_classification")) test_detector_classification(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile);
